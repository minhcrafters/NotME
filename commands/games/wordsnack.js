...
module.exports = class Command extends Commando.Command {
	constructor() {
		super('wordsnack', {
			aliases: ['wordsnack', 'wordssnack'],
			category: 'games',
			channel: 'guild',
			description: 'Play Words Snack, a Discord\'s new game.',
			args: [
				{
					id: 'voiceChannel',
					type: 'voiceChannel'
	...
	async exec(message, { voiceChannel }) {
		...
		this.client.discordTogether.createTogetherCode(voiceChannel.id, 'wordsnack').then(async (invite) => {
			const embed = new Discord.MessageEmbed()
				.setAuthor('Words Snack')
